...
public class JSFUtils {
   ...
   */
    ...
    public static String getStringFromBundle(String bundle, String key,
                                             String[] params) {
        String value =
            resolveExpressionAsString("#{" + bundle + "." + key + "}");
        return replaceParameters(value, params);
    }

    private static String replaceParameters(String val, String[] params) {
        ...
        if (params != null) {
            ...
            for (int i = 0; i < len; i++) {
                if ((c = val.charAt(i)) == '{') {
                    int j = i + 1;
                    String pIndex = "";
                    while (j < len && Character.isDigit(val.charAt(j))) {
                        pIndex += val.charAt(j);
                        j++;
                    }
                    if (j < len && val.charAt(j) == '}') {
                        int pindex = Integer.parseInt(pIndex);
                        if (pindex < params.length) {
                            rez.append(params[pindex]);
                        }
                        i = j;
                    } else {
                        rez.append(val.subSequence(i, j));
                    }
                } else {
                    rez.append((char)c);
                }
